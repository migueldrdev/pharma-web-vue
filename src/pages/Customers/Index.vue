  ...
  <q-page class="q-pa-md">
    ...
    <q-dialog v-model="showForm" :maximized="$q.screen.lt.md">
      <q-card style="width:500px;max-width:95vw">
        <q-bar class="bg-primary text-white"><div>{{ isEdit ? 'Editar' : 'Nuevo' }} Cliente</div><q-space /><q-btn dense flat icon="close" v-close-popup /></q-bar>
        <q-card-section>
    ...
    <AppConfirmDialog v-model="showDelete" title="Eliminar cliente" :message="deleteMessage" color="negative" confirm-label="Eliminar" @confirm="void handleDelete()" />
  </q-page>
</template>
...
async function load() {
  loading.value = true;
  try {
    const r = await fetchHttpResource({ path: '/client', method: 'get' as never });
...
function openCreate() {
  form.value = { id: null, name: '', email: '', phone: '', address: '', document_number: '' };
  isEdit.value = false;
  showForm.value = true;
}
...
async function save() {
  ...
  saving.value = true;
  try {
    const m = isEdit.value ? 'put' : 'post';
    const r = await fetchHttpResource({
      path: '/client' + (isEdit.value ? `/${form.value.id}` : ''),
      method: m as never,
...
async function handleDelete() {
  ...
  try {
    const r = await fetchHttpResource({
      path: `/client/${deleteTarget.value.id}`,
