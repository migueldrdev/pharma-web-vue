...
export function salesChartSeries(salesData: number[], goalData: number[]): SeriesOptionsType[] {
  return [
...
export function barChartSeries(name: string, data: number[]): SeriesOptionsType[] {
  return [{ name, type: 'bar', data, dataLabels: { enabled: true } }];
}

export function pieChartSeries(data: { name: string; y: number }[]): SeriesOptionsType[] {
  return [{ name: 'Productos', type: 'pie', data, dataLabels: { enabled: true, format: '{point.name}: {point.percentage:.1f}%' } }];
}

export function columnChartSeries(incomeData: number[], expenseData: number[]): SeriesOptionsType[] {
  return [
