...
- **Nunca** usar colores personalizados; usar la paleta unificada de Quasar (sobrescribiendo variables)
- **Nunca** usar `any` en TypeScript; preferir `unknown` o genéricos con tipos específicos
- **Siempre** usar `defineProps<T>()` y `defineEmits<T>()` con tipos en componentes Vue
